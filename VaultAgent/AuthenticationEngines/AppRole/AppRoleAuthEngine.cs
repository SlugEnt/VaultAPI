	...
	public class AppRoleAuthEngine : VaultAuthenticationBackend
	{
		...
		public async Task<List<string>> ListRoles () {
			...
			try {
				...
				VaultDataResponseObject vdro = await _parent._httpConnector.GetAsync(path, "ListRoles",contentParams);
				if (vdro.Success) {
...
*/
	    ...
	    public async Task<List<string>> ListSecretIDAccessors(string roleName)
	    {
	        ...
	        try
	        {
	            ...
	            VaultDataResponseObject vdro = await _parent._httpConnector.GetAsync(path, "ListSecretIDAccessors", contentParams);
	            if (vdro.Success)
	            {
	                string js = vdro.GetJSONPropertyValue(vdro.GetDataPackageAsJSON(), "keys");
	                List<string> keys = VaultUtilityFX.ConvertJSON<List<string>>(js);
...
//				return await GetMyTokenInfo();
		    ...
		    catch (VaultInvalidDataException e)
		    {
		        ...
		        {
                    VaultInvalidDataException newEx = new VaultInvalidDataException("The secret ID supplied is either not a valid Secret ID or it is not associated with the RoleID supplied.  The secretID must be valid and it must be associated with the provided RoleID.",e);
