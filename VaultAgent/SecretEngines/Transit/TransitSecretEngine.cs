...
namespace VaultAgent.SecretEngines
{
    public class TransitSecretEngine : VaultSecretBackend
	{
		...
		public TransitSecretEngine(string backendMountName, string backendMountPath, VaultAgentAPI vaultAgentAPI) : base(backendMountName, backendMountPath, vaultAgentAPI) {
		...
		public async Task<TransitDataKey> GenerateDataKey(string keyName, bool returnCipherAndPlainText = false, string context = "", int bits = 256) {
			...
			if ((bits != 128) && (bits != 256) && (bits != 512)) { throw new ArgumentOutOfRangeException("bits", "Bits value can only be 128, 256 or 512."); }
