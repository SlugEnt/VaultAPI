...
namespace VaultAgent
{
	public class TimeUnit
	{
		...
		public TimeUnit( int seconds) {
			value = seconds;
		}

		public TimeUnit (string timeValue) {
			// Break into number and text parts.
			string timeDuration = new string(timeValue.TakeWhile(c => !Char.IsLetter(c)).ToArray());
			string timeNumber = new string(timeValue.TakeWhile(Char.IsDigit).ToArray());

			// Validate
			double val;
			if (double.TryParse(timeNumber, out val)) {
				switch (timeDuration) {
					case "d":
						value = ConvertDaysToSeconds(val);
						break;
					case "m":
						value = ConvertMinutesToSeconds(val);
						break;
					case "h":
						value = ConvertHoursToSeconds(val);
						break;
					case "s":
						value = val;
						break;
					case "w":
						value = ConvertDaysToSeconds(val * 7);
						break;
				}
			}
		}
