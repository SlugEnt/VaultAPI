...
namespace VaultAgentTests {
    ...
    public class PolicyTests {
        ...
        [TestCase (5, "secret5", "path1/path2/path3/", "secret5", "path1/path2/path3/*", true)]
        public void VPPI_DefaultConstructor_BackendPathIsPrefix_Works (int id,
                                                                       string backend,
                                                                       string path,
                                                                       string expectedBE,
                                                                       string expectedPath,
                                                                       bool expectedSubFolderType) {
        ...
        public async Task Vault_CanReadMultiplePathPolicy () {
            ...
            foreach ( VaultPolicyPathItem item in vpNew.PolicyPaths.Values ) {
                ...
                else if ( item.SecretPath == path2 ) { Assert.AreEqual (vpi2.Denied, item.Denied, "a40:  Denied property not expected value."); }
        ...
        public async Task Vault_Validate_Complex_KV2PolicyTest () {
            ...
            foreach ( VaultPolicyPathItem item in vpNew.PolicyPaths.Values ) {
                ...
                else if ( item.SecretPath == path2 ) { Assert.AreEqual (vpi2.Denied, item.Denied); }
                else if ( item.SecretPath == path3 ) {
        ...
        public void Vault_ReadOfNonExistentPolicy_ResultsInExpectedError () {
            VaultInvalidPathException e =
                Assert.ThrowsAsync<VaultInvalidPathException> (async () => await _vaultSystemBackend.SysPoliciesACLRead ("NonExistentPolicy"));
        ...
        public async Task Policy_Delete_InvalidPolicyName_ReturnsTrue () { Assert.True (await _vaultSystemBackend.SysPoliciesACLDelete ("invalidName")); }
        ...
        [TestCase (2, "A", "path2/", "A/data/path2/*", Description =
            "This is the expected format of parameters, backend, path without KV2 prefix and securing subfolders.")]
        [TestCase (3, "A", "path3/path4", "A/data/path3/path4")]
        [TestCase (4, "A", "data/path1", "A/data/path1", Description =
            "Validates that if the KV2 prefix path is specified in the path, that it is stripped out.")]
        [TestCase (5, "B", "metadata/path2/*", "B/data/path2/*", Description =
            "Validates that if the KV2 prefix path is specified in the path, that it is stripped out.")]
        ...
        public async Task FullCycleTest_OnPrimaryFolder () {
            ...
            Assert.True (await secEng.SaveSecret (secret, KV2EnumSecretSaveOptions.OnlyOnExistingVersionMatch, readSecret.Version),
